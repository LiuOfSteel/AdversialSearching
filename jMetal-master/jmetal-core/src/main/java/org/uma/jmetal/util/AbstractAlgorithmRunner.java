...
public abstract class AbstractAlgorithmRunner {
  ...
  public static void CustmizedPrintFinalSolutionSet(List<? extends Solution<?>> population, int populationSize, int MaxEvaluations, String ZDT) {

    new SolutionListOutput(population)
            .setSeparator("\t")
            .setVarFileOutputContext(new DefaultFileOutputContext("VAR_" + ZDT + populationSize + "_" + MaxEvaluations + ".tsv"))
            .setFunFileOutputContext(new DefaultFileOutputContext("FUN_" + ZDT + populationSize + "_" + MaxEvaluations + ".tsv"))
            .print();

    JMetalLogger.logger.info("Random seed: " + JMetalRandom.getInstance().getSeed());
    JMetalLogger.logger.info("Objectives values have been written to file FUN_" + ZDT + populationSize + "_" + MaxEvaluations + ".tsv");
    JMetalLogger.logger.info("Variables values have been written to file VAR_" + ZDT + populationSize + "_" + MaxEvaluations + ".tsv");
  }
  /**
